...
{
    public Task<IReadOnlyList<ScanItem>> ScanAsync(string input, int recursiveDepth, string? temporaryDirectory, IProgress<OperationProgress>? progress, CancellationToken cancellationToken, IProgress<ScanItem>? completed = null, bool inspectSimple = false)
    {
        var id = Guid.NewGuid();
        return OperationLog.RunAsync(logger, "Scan", id, input, () => ScanCoreAsync(input, recursiveDepth, temporaryDirectory, progress, id, cancellationToken, completed, inspectSimple), cancellationToken, results => results.Count == 0 ? OperationStatus.Skipped : results.All(Failed) ? OperationStatus.Failed : results.Any(Failed) ? OperationStatus.Partial : OperationStatus.Completed);
    }

    private static bool Failed(ScanItem item) => item.Error is not null || item.Analysis?.Verdict == AnalysisVerdict.AnalysisFailed;
    private async Task<IReadOnlyList<ScanItem>> ScanCoreAsync(string input, int recursiveDepth, string? temporaryDirectory, IProgress<OperationProgress>? progress, Guid id, CancellationToken cancellationToken, IProgress<ScanItem>? completed, bool inspectSimple)
    {
        ...
        if (inspectSimple)
        {
            var candidates = results.Select((item, index) => (item, index)).Where(candidate => candidate.item.Error is null && candidate.item.Analysis?.Verdict == AnalysisVerdict.SimpleFel).ToArray();
