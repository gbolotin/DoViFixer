...
{
    public CleanupPlan Plan(string input, int recursiveDepth) => new(Guid.NewGuid(), Array.AsReadOnly(discovery.Discover(input, recursiveDepth, cleanup: true).Select(files.Identify).ToArray()));
