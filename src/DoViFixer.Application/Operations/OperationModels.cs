...
{
    public OperationStatus Status => Files.Count == 0 ? OperationStatus.Skipped : Files.Any(f => f.Status == OperationStatus.Cancelled) ? OperationStatus.Cancelled : Files.All(f => f.Status == OperationStatus.Completed) ? OperationStatus.Completed : Files.Any(f => f.Status is OperationStatus.Completed or OperationStatus.Partial) ? OperationStatus.Partial : OperationStatus.Failed;
}
