...
internal sealed class SettingsStore(StorageOptions options, ILogger<SettingsStore> logger) : ISettingsStore
{
    ...
    public async Task<UserSettings> ReadAsync(CancellationToken cancellationToken)
    {
        ...
        return await JsonSerializer.DeserializeAsync<UserSettings>(stream, json, cancellationToken) ?? throw new InvalidDataException("Settings file is empty or invalid; repair settings.json before continuing.");
    }
    ...
    public async Task UpdateAsync(Func<UserSettings, UserSettings> update, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using var audit = logger.BeginScope(new Dictionary<string, object>
            {
                ["LogKind"] = "Audit"
            });
            ...
            foreach (var tool in previous.ToolPaths.Keys.Union(updated.ToolPaths.Keys))
            {
                ...
                if (before != after)
                {
                    logger.LogInformation("Setting {Setting} changed from {PreviousValue} to {NewValue} in {SettingsPath}", "ToolPaths." + tool, before, after, options.SettingsPath);
                }
            }
        }
        finally
