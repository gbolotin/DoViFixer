...
internal sealed class DependencyInstaller(StorageOptions storage, IProcessRunner processes, HttpClient http, ILogger<DependencyInstaller> logger) : IDependencyInstaller
{
    public async Task<InstallationPlan> PrepareAsync(DependencyReport report, CancellationToken cancellationToken)
    {
        ...
        var replacements = report.Tools.Where(t => t.State is DependencyState.Incompatible or DependencyState.Unusable && items.Any(i => i.Tools.Contains(t.Tool))).ToArray();
        return new(Guid.NewGuid(), items.AsReadOnly(), unavailable.AsReadOnly(), replacements);
    }

    public async Task<IReadOnlyList<InstallationOutcome>> InstallAsync(InstallationPlan approvedPlan, IProgress<OperationProgress>? progress, CancellationToken cancellationToken)
    {
        var results = new List<InstallationOutcome>();
        foreach (var item in approvedPlan.Items)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var package = InstallationCatalog.Packages.SingleOrDefault(p => p.Id == item.Id) ?? throw new InvalidOperationException("Installation plan contains an unknown package.");
            ...
            try
            {
                ...
                if (item.Provider == InstallationProvider.WinGet)
                {
                    ...
                    await processes.RunAsync(new(WinGetPath(), new[]
                    {
                        "install", "--id", item.Id, "--exact", "--version", item.Version, "--source", "winget", "--scope", "user", "--installer-type", "zip", "--architecture", "x64", "--accept-package-agreements", "--accept-source-agreements", "--disable-interactivity"
                    }, Timeout: TimeSpan.FromMinutes(30)), cancellationToken);
                }
                else
                {
    ...
    private async Task<bool> PackageAvailableAsync(ToolPackage package, CancellationToken cancellationToken)
    {
        try
        {
            var result = await processes.RunAsync(new(WinGetPath(), new[]
            {
                "show", "--id", package.Id, "--exact", "--version", package.Version, "--source", "winget", "--accept-source-agreements", "--disable-interactivity"
            }, Timeout: TimeSpan.FromSeconds(30)), cancellationToken);
            return result.Output.Contains(package.Url, StringComparison.OrdinalIgnoreCase) && result.Output.Contains(package.Sha256, StringComparison.OrdinalIgnoreCase);
        }
        catch (Exception ex)when (ex is not OperationCanceledException)
        {
            return false;
