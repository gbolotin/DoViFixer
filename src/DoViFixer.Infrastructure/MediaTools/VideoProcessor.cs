...
internal sealed class VideoProcessor(IToolCatalog tools, IProcessRunner processes, TimeProvider time, ILogger<VideoProcessor> logger) : IVideoProcessor
{
    public async Task ConvertAsync(MediaInfo media, ConversionTarget target, ITemporaryWorkspace workspace, string stagedOutput, IProgress<OperationProgress>? progress, Guid operationId, CancellationToken cancellationToken, bool safe = false)
    {
        ...
        string[] ConversionArguments(string input) => target == ConversionTarget.Profile81 ? new[]
        {
            "-m",
            "2",
            "convert",
    ...
    public async Task RestoreAsync(MediaInfo media, ArchiveManifest? manifest, ITemporaryWorkspace workspace, string stagedOutput, CancellationToken cancellationToken)
    {
        ...
        await RunDoviAsync(new[]
        {
            "mux", "--no-add-aud", "--bl", clean, "--el", workspace.File("el.hevc"), "-o", restored
        }, cancellationToken);
        File.Delete(clean);
        ...
        await RunDoviAsync(new[]
        {
            "demux", "--el-only", "-i", restored, "-e", verifiedEl
        }, cancellationToken);
    ...
    {
        "--gui-mode", media.Source.Path, "tracks", $"{media.VideoTrackId}:{output}"
    }, AllowWarnings: true, OutputLine: outputLine), cancellationToken);
    internal Task RunDoviAsync(string[] arguments, CancellationToken cancellationToken) => processes.RunAsync(new(tools.GetPath(NativeTool.DoviTool), arguments), cancellationToken);
    private async Task RemuxAsync(MediaInfo media, string video, string output, ITemporaryWorkspace workspace, CancellationToken cancellationToken, Action<string>? outputLine = null)
    {
        ...
        {
            media.Source.Path, "timestamps_v2", $"{media.VideoTrackId}:{timestamps}"
        ...
        var videoProperties = identification.RootElement.GetProperty("tracks").EnumerateArray().Single(t => t.GetProperty("id").GetInt32() == media.VideoTrackId).GetProperty("properties");
        string tags = workspace.File("source-tags.xml");
        await processes.RunAsync(new(tools.GetPath(NativeTool.MkvExtract), new[]
        {
            media.Source.Path, "tags", tags
        }, AllowWarnings: true), cancellationToken);
        ...
        var arguments = new List<string>
        {
            ...
            output,
            "--track-order",
            string.Join(",", media.Tracks.Select(t => t.Type == "video" ? "1:0" : $"0:{t.Id}")),
            "--no-video",
            "--no-track-tags"
        };
        ...
        {
            "--timestamps", $"0:{timestamps}", "--language", $"0:{original.Language}", "--track-name", $"0:{original.Name}", "--default-track-flag", $"0:{(original.Default ? 1 : 0)}", "--forced-display-flag", $"0:{(original.Forced ? 1 : 0)}"
        });
        ...
        {
            var trackTags = document.Root?.Elements("Tag").Where(t => t.Elements("Simple").Any() && t.Element("Targets")?.Elements("TrackUID").Any(u => u.Value == track.Uid) == true).ToArray() ?? [];
