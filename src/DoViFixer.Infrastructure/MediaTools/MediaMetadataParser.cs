...
internal static class MediaMetadataParser
{
    internal static MediaInfo Parse(FileIdentity source, string mkvJson, string mediaInfoJson)
    {
        ...
        if (Text(root.GetProperty("container"), "type") != "Matroska" || (root.TryGetProperty("errors", out var errors) && errors.GetArrayLength() != 0))
        {
        ...
        var tracks = tracksJson.EnumerateArray().Select(t =>
        {
            ...
            return new MediaTrack(t.GetProperty("id").GetInt32(), Text(t, "type"), Text(t, "codec"), Text(p, "language_ietf", Text(p, "language", "und")), Text(p, "track_name"), Bool(p, "default_track"), Bool(p, "forced_track"), Text(p, "uid"));
        }).ToArray();
        ...
        var profile = !hdr.Contains("Dolby Vision", StringComparison.OrdinalIgnoreCase) && !match.Success ? DolbyVisionProfile.None : match.Success ? match.Groups[1].Value switch
        {
        ...
        return new(source, profile, Text(miVideo, "Format", video.Codec), video.Id, (int)(Number(miVideo, "Width") ?? 0), (int)(Number(miVideo, "Height") ?? 0), (long? )Number(miVideo, "FrameCount"), Number(miVideo, "FrameRate"), Number(miVideo, "Duration") ?? Number(container, "duration") / 1_000_000_000d, (long)(Number(properties, "minimum_timestamp") ?? 0), Number(miVideo, "MaxCLL"), Array.AsReadOnly(tracks), root.TryGetProperty("attachments", out var attachments) ? attachments.GetArrayLength() : 0, root.TryGetProperty("chapters", out var chapters) ? chapters.EnumerateArray().Sum(c => (int)(Number(c, "num_entries") ?? 0)) : 0, Text(container, "title"), mkvJson);
    }

    internal static string Text(JsonElement element, string name, string fallback = "") => element.ValueKind == JsonValueKind.Object && element.TryGetProperty(name, out var value) && value.ValueKind != JsonValueKind.Null ? value.ToString() : fallback;
    internal static double? Number(JsonElement element, string name) => double.TryParse(Text(element, name), NumberStyles.Float, CultureInfo.InvariantCulture, out double value) && double.IsFinite(value) ? value : null;
