...
internal static class DeepInspectionParser
{
    ...
    internal static string Filter(string range) => $"zscale=pin=bt2020:tin=smpte2084:min=bt2020nc:rin={range}:p=bt2020:t=linear:m=gbr:r=full:npl=10000," + "format=gbrpf32le,zscale=p=bt2020:t=linear:m=bt2020nc:r=full,format=yuv444p16le," + "signalstats,metadata=mode=print:key=lavfi.signalstats.YMAX:file=brightness.txt";
    internal static string ReadHdr10Range(string json)
    {
        ...
        if (MediaMetadataParser.Text(stream, "color_transfer") != "smpte2084" || MediaMetadataParser.Text(stream, "color_primaries") != "bt2020" || MediaMetadataParser.Text(stream, "color_space") != "bt2020nc")
        {
    ...
    internal static async Task<RpuEvidence> CompareAsync(Stream rpu, TextReader measurements, CancellationToken cancellationToken)
    {
        ...
        var evidence = await RpuParser.ParseAsync(rpu, AnalysisMethod.DeepInspection, cancellationToken, (index, pq) =>
        {
            ...
            if (header is null || !header.StartsWith(expected + " ", StringComparison.Ordinal) || value is null || !value.StartsWith("lavfi.signalstats.YMAX=", StringComparison.Ordinal) || !int.TryParse(value.AsSpan("lavfi.signalstats.YMAX=".Length), NumberStyles.None, CultureInfo.InvariantCulture, out int code) || code is < 0 or > 65535)
            {
        ...
        {
            Brightness = new(compared, expanded, basePeak, maxDelta, maxFrame, threshold)
        };
    }
}
