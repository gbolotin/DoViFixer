...
internal sealed class MkvProgress(IProgress<OperationProgress>? progress, Guid operationId, string stage, string file)
{
    ...
    public void Report(string line)
    {
        ...
        if (line.StartsWith(prefix, StringComparison.Ordinal) && int.TryParse(line.AsSpan(prefix.Length).Trim().TrimEnd('%'), NumberStyles.None, CultureInfo.InvariantCulture, out int percent) && percent is >= 0 and <= 100 && percent > lastPercent)
        {
