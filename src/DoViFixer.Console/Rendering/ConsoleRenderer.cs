...
public sealed class ConsoleRenderer : IProgress<OperationProgress>, IDisposable
{
    ...
    {
        if (color is null || System.Console.IsOutputRedirected || !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("NO_COLOR")) || Environment.GetEnvironmentVariable("TERM") == "dumb")
        {
            Write(message);
            return;
        }
