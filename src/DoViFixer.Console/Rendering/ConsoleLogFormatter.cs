...
internal sealed class ConsoleLogFormatter : ITextFormatter
{
    ...
    public void Format(LogEvent logEvent, TextWriter output)
    {
        ...
        if (logEvent.MessageTemplate.Text.Contains(elapsedToken, StringComparison.Ordinal) && logEvent.Properties.TryGetValue("ElapsedMilliseconds", out var value) && value is ScalarValue
        {
        ...
        && double.IsFinite(milliseconds) && milliseconds >= 0 && milliseconds < TimeSpan.MaxValue.TotalMilliseconds)
        {
            ...
            var properties = logEvent.Properties.Where(p => p.Key != "ConsoleElapsed").Select(p => new LogEventProperty(p.Key, p.Value)).Append(new LogEventProperty("ConsoleElapsed", new ScalarValue(FormatElapsed(milliseconds))));
