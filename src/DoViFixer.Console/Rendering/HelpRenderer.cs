...
internal sealed class HelpRenderer(ConsoleRenderer renderer)
{
    ...
    public void Write()
    {
        ...
        Options(("-r, --recursive [depth]", "Include subfolders. Default depth with -r: 5; explicit depth: 0 to 100."), ("--candidates-only", "Show conversion candidates only."), ("--inspect-simple", "Deep-inspect every Simple FEL candidate after scanning. Decodes all base-layer frames; slow."), ("--json", "Write structured JSON for scripts."));
        Examples(("scan", "Scan the current folder"), ("scan --recursive", "Recursive scan, default depth 5"), ("scan -r", "Same, short form"), ("scan --recursive 2", "Recursive scan, depth 2"), ("scan \"D:\\Movies\" -r 3", "Scan a specific folder, depth 3"), ("scan -r --candidates-only", "Show only conversion candidates"));
        Command("inspect <file.mkv> [--deep] [--json]", "Analyze one file in detail. Deep inspection compares every decoded base-layer frame's luminance with RPU L1 metadata to look for possible FEL brightness expansion.");
        Options(("--deep", "Run full frame-by-frame inspection (slow)."));
        Examples(("inspect \"Movie.mkv\"", "Inspect one movie"), ("inspect \"Movie.mkv\" --deep", "Measure possible brightness expansion"));
        Group("File Conversion");
        Command("convert [files-or-directories...] [-r [depth]] [--hdr10] [--include-simple] [--force] [--backup] [--delete] [--safe] [--output directory] [--temp directory] [--plan | --yes]", "Convert eligible Profile 7 files to Dolby Vision Profile 8.1 (default) or HDR10. Accepts multiple files and folders; no paths means the current directory.");
        Options(("--hdr10", "Produce HDR10 instead of Dolby Vision Profile 8.1."), ("--include-simple", "Include Simple FEL candidates in flag-based selection (--plan / --yes)."), ("-f, --force", "Permit detected Complex FEL; picture data will be lost. Failed or unknown analysis stays blocked."), ("-b, --backup", "Save a .dovi enhancement archive before conversion."), ("--delete", "Rename the original to .bak.dovi_convert, reuse its filename, then delete that backup after verification."), ("-s, --safe", "Force disk extraction. Default: streaming with disk fallback."), ("-o, --output directory", "Write output to the selected directory."));
        ...
        Examples(("convert \"Movie.mkv\" --plan", "Preview the conversion plan"), ("convert \"D:\\Movies\" -r", "Review and approve a folder batch"), ("convert \"Movie.mkv\" --hdr10 --backup", "Create HDR10 and an enhancement archive"));
        Group("Backup & Restore");
        Command("backup <file.mkv> [--output directory] [--temp directory] [--plan | --yes]", "Save the enhancement layer and restoration metadata in a .dovi archive for later restoration.");
        Examples(("backup \"Movie.mkv\" --plan", "Preview archive creation"), ("backup \"Movie.mkv\"", "Create an archive after approval"));
        Command("restore <base.mkv> [--source <archive.dovi>] [--output directory] [--temp directory] [--plan | --yes]", "Restore Profile 7 from a compatible base file and enhancement archive. Finds the adjacent .dovi archive by default; EL-only upstream archives are also accepted.");
        Options(("--source archive.dovi", "Choose an archive explicitly instead of the adjacent archive."));
        Examples(("restore \"Movie - DV P8.1.mkv\" --plan", "Preview restoration"), ("restore \"Base.mkv\" --source \"Movie.dovi\"", "Restore using a chosen archive"));
        Group("Cleanup");
        Command("cleanup [file-or-directory] [-r [depth]] [--delete-backups [--yes]]", "List .dovi archives and .bak.dovi_convert backups. Defaults to the current directory and only lists files unless deletion is requested.");
        Options(("--delete-backups", "Request permanent deletion of listed backups, with confirmation."), ("--yes", "Approve deletion without prompting; requires --delete-backups."));
        Examples(("cleanup \"D:\\Movies\" -r", "List backups without deleting them"));
        Group("Dependencies");
        Command("dependencies check [Tool ...] [--json]", "Check native tool availability and compatibility. Omit tool names to check all tools.");
        Command("dependencies install [Tool ...] [--yes]", "Prepare an installation plan for missing, unusable, or incompatible tools, then ask for approval. Validated tool paths are saved after installation.");
        Options(("--yes", "Approve this software installation plan without prompting."), ("--repair", "Accepted for compatibility; repair is already included when needed."));
        Paragraph("Tool names: FFmpeg, FFprobe, MkvMerge, MkvExtract, MediaInfo, DoviTool.");
        Examples(("dependencies check", "Check all native tools"), ("dependencies check FFmpeg DoviTool --json", "Check selected tools as JSON"), ("dependencies install", "Review and approve required installations"));
        Group("Settings");
        ...
        Table("Code", "Meaning", [("0", "Success"), ("1", "Failure or partial completion"), ("2", "Invalid arguments"), ("3", "Dependencies unavailable"), ("4", "Declined or no selected work"), ("130", "Cancelled")]);
    }
    ...
    private void Examples(params (string, string)[] rows) => Table("Example (run in your terminal)", "What it does", rows.Select(row => ($"DoViFixer.Console {row.Item1}", row.Item2)).ToArray());
