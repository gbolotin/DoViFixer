...
public sealed class ConversionCommand(ConversionPlanner planner, BatchConversionService conversion, ConsoleRenderer renderer, ConsoleInteraction interaction) : IConsoleCommand
{
    ...
    {
        var request = new ConversionRequest(command.Arguments.FirstOrDefault() ?? Environment.CurrentDirectory, command.Depth, command.Has("hdr10") ? ConversionTarget.Hdr10 : ConversionTarget.Profile81, command.Value("output"), command.Value("temp"), command.Has("include-simple"), command.Has("force"), command.Has("backup"), command.Has("safe"), command.Has("delete"), command.Arguments.Skip(1).ToArray());
        ...
        var planningRequest = interactiveApproval ? request with
        {
            IncludeSimple = true,
            ForceComplex = true
        }
        ...
        foreach (var plan in planned.Plans)
        {
            ...
            {
                plan.Analysis.Media.Source, plan.Output, plan.Archive, plan.Target, plan.ScratchBytes, plan.TemporaryDirectory, plan.Decision, plan.Safe, plan.DeleteBackup, request.ForceComplex, request.IncludeSimple
            });
