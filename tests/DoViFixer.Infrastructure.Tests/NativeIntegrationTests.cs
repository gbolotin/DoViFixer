...
public sealed class NativeIntegrationTests
{
    ...
    public async Task NativeMetadataRecognizesProfile7Directory()
    {
        ...
        catalog.Refresh(new[]
        {
            new DependencyStatus(NativeTool.MediaInfo, DependencyState.Ready, mediaInfo, "test", "explicit test tool"), new DependencyStatus(NativeTool.MkvMerge, DependencyState.Ready, mkvMerge, "test", "explicit test tool")
        });
        var probe = new MediaProbe(catalog, new ProcessRunner(NullLogger<ProcessRunner>.Instance), new FileOperations(), NullLogger<MediaProbe>.Instance);
    ...
    public async Task NativeFixtureConvertsBacksUpAndRestoresWithIdenticalBaseAndEnhancementPayloads(bool includeMetadata, bool safe, bool failStreaming)
    {
        ...
        tools.Refresh(new[]
        {
            new DependencyStatus(NativeTool.MediaInfo, DependencyState.Ready, "fixture-mediainfo", "fixture", "fixture")
        });
        var runner = new ProcessRunner(NullLogger<ProcessRunner>.Instance);
        var files = new FileOperations();
        var factory = new TemporaryWorkspaceFactory(files, NullLogger<TemporaryWorkspaceFactory>.Instance);
        ...
        await runner.RunAsync(new(tools.GetPath(NativeTool.DoviTool), new[]
        {
            "-m", "1", "convert", rawFixture, "-o", profile7
        }), default);
        var muxArguments = new List<string>
        {
            "-o",
            input,
            "--language",
        ...
        if (includeMetadata)
        {
            ...
            muxArguments.AddRange(new[]
            {
                "--title", "Fixture title", "--chapters", chapters, "--attachment-mime-type", "text/plain", "--attach-file", attachment, "--language", "0:en", "--track-name", "0:Test audio", audio, "--language", "0:he", "--track-name", "0:Test subtitles", "--default-track-flag", "0:0", subtitles
            });
        }

        await runner.RunAsync(new(tools.GetPath(NativeTool.MkvMerge), muxArguments), default);
        var identification = await runner.RunAsync(new(tools.GetPath(NativeTool.MkvMerge), new[]
        {
            "-J", input
        }), default);
        ...
        var convertedIdentification = await runner.RunAsync(new(tools.GetPath(NativeTool.MkvMerge), new[]
        {
            "-J", output
        }), default);
        ...
        var convertedMedia = MediaMetadataParser.Parse(files.Identify(output), convertedIdentification.Output, mi)with
        {
        ...
        {
            input, "timestamps_v2", "0:" + beforeTimestamps
        }), default);
        await runner.RunAsync(new(tools.GetPath(NativeTool.MkvExtract), new[]
        {
            output, "timestamps_v2", "0:" + afterTimestamps
        }), default);
        ...
        var restoredMedia = media with
        {
            Source = files.Identify(restored)
        };
        ...
        await runner.RunAsync(new(tools.GetPath(NativeTool.DoviTool), new[]
        {
            "extract-rpu", convertedRaw, "-o", rpu
        }), default);
        await runner.RunAsync(new(tools.GetPath(NativeTool.DoviTool), new[]
        {
            "export", "-i", "converted.rpu", "-d", "all=converted.json"
        }, verificationWorkspace.DirectoryPath), default);
        ...
        {
            Source = files.Identify(hdrOutput)
        }, hdrRaw, default);
        await processor.RunDoviAsync(new[]
        {
            "remove", hdrRaw, "-o", hdrClean
        }, default);
        Assert.AreEqual(manifest.BaseLayerSha256, await VideoProcessor.HashAsync(hdrClean, default));
        var noRpu = await runner.RunAsync(new(tools.GetPath(NativeTool.DoviTool), new[]
        {
            "extract-rpu", hdrRaw, "-o", hdrWorkspace.File("absent.rpu")
        ...
        {
            ConversionTarget.Profile81,
            ConversionTarget.Hdr10
        }
        ...
        )
        {
            ...
            string serviceOutput = safe ? files.PrepareOutputPath(serviceInput, null, target == ConversionTarget.Profile81 ? " - DV P8.1.mkv" : " - HDR10.mkv") : serviceInput;
            var result = await service.ExecuteAsync(new(Guid.NewGuid(), analysis, target, serviceOutput, serviceArchive, workspace.DirectoryPath, 16 * 1024 * 1024, "fixture", Safe: safe, DeleteBackup: !safe), null, default);
            Assert.AreEqual(OperationStatus.Completed, result.Status, result.Message);
    ...
    private sealed class FixtureMediaInfoRunner(IProcessRunner native, IReadOnlyDictionary<string, string> fixtures, string? defaultJson = null) : IProcessRunner
    {
        ...
        public Task<ProcessResult> RunAsync(ProcessRequest request, CancellationToken cancellationToken) => request.Executable == "fixture-mediainfo" ? Task.FromResult(new ProcessResult(0, fixtures.TryGetValue(request.Arguments[^1], out var json) ? json : defaultJson ?? throw new InvalidOperationException("Missing MediaInfo fixture"), "")) : native.RunAsync(request, cancellationToken);
    }

    private sealed class RecordingProgress : IProgress<DoViFixer.Application.Operations.OperationProgress>
    {
        public List<DoViFixer.Application.Operations.OperationProgress> Values
        {
            get;
        }
        = [];
