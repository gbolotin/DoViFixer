...
| convert | Profile 8.1 and HDR10; full analysis before planning; `--include-simple`, `--force`, `--backup`, `--temp`, `--output`, `--yes`, `--plan` | Disk path is the default; streaming/fallback selection is deferred. `--safe` is accepted. Unknown analysis is never forced. One video track and Matroska input required. |
...
| Retention | Original MKV always remains in place; new output gets a suffix | Does not rename originals to `.bak.dovi_convert` and does not support upstream conversion `--delete`. Original retention and EL archives remain separate. |
...
| restore | Validates manifest/payload, checks exact BL pairing, reconstructs EL, remuxes and verifies | Legacy `el.hevc`-only TAR requires explicit unverified-pairing consent. Container byte identity is not expected. |
