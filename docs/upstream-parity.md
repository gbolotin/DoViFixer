...
| inspect | Complete disk extraction and streamed per-frame RPU JSON analysis; optional `--deep` decodes HDR10 base-layer luminance per frame | Full RPU count must match ffprobe video packet count. Deep mode also requires complete decoded-frame/L1 coverage and explicit HDR10 signaling. See [Deep Inspection](deep-inspection.md) for measurement and interpretation limits. |
...
| Retention | Default keeps the original path unchanged and publishes `<stem> - DV P8.1.mkv` or `<stem> - HDR10.mkv` beside it (or under `--output`) | Intentional upstream difference. Explicit `--delete` uses the replacement/backup workflow and deletes the backup only after verified publication. Failed/cancelled replacement restores the original path; default conversion requires no source recovery. EL archives are retained; outputs are never overwritten. |
